...
这份文档说明 FrontEnd 如何以生产等价方式联调 BackEnd 的 REST BFF 服务.

当前口径如下:

- 不使用 mock
- 测试必须真实调用 `RiskAgent-BackEnd`
- 页面联调必须跑在本地 K8s
- FrontEnd 和 BackEnd 都必须运行在 Pod 中
...
- `tests/unit/riskagent-api.test.ts`
...
验证 FrontEnd 真实调用 BackEnd 的以下接口:
...
- BackEnd Pod 真实连接 MySQL Redis Chroma
...
1. 构建前端和后端本地镜像
2. 用 Helm 在本地 K8s 中部署 BackEnd MySQL Redis Chroma FrontEnd
...
- FrontEnd 和 BackEnd 都运行在同一个本地 K8s namespace 的 Pod 中
...
- `RISKAGENT_K8S_NAMESPACE`: 本地 K8s 验收使用的 namespace
- `RISKAGENT_K8S_BACKEND_RELEASE`: BackEnd Helm release 名称
- `RISKAGENT_K8S_FRONTEND_RELEASE`: FrontEnd Helm release 名称
- `RISKAGENT_K8S_FRONTEND_PORT_FORWARD`: 本地映射端口 默认 `4173`
- `RISKAGENT_K8S_BACKEND_IMAGE`: 后端镜像名
- `RISKAGENT_K8S_FRONTEND_IMAGE`: 前端镜像名
